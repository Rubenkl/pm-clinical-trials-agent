...
def _generate_analysis_data(subjects: list, limit: int = 50):
    ...
    for subject_id in subjects[:limit]:
        ...
        analysis = {
            ...
            "severity": "critical" if subject_hash % 5 == 0 else "major" if subject_hash % 3 == 0 else "minor",
            **template
        }
        ...
        if subject_hash % 2 == 0:
            second_template = analysis_templates[(subject_hash + 1) % len(analysis_templates)]
...
def _format_queries_for_frontend(queries: list, analyses: list):
    ...
    for i, query in enumerate(queries):
        ...
        formatted_query = {
            ...
            "query_type": "data_clarification" if analysis["category"] == "data_discrepancy" else "source_verification",
            ...
            "assigned_to": "site_coordinator" if query.get("priority") == "high" else "data_manager",
...
def _calculate_query_statistics(queries: list, sites: list):
    ...
    overdue_queries = sum(1 for q in queries if q["due_date"] < "2025-01-10T00:00:00" and q["status"] != "resolved")
    critical_queries = sum(1 for q in queries if q["severity"] == "critical")
    
    # Count by categories using dictionary comprehensions
    status_counts = {status: sum(1 for q in queries if q["status"] == status) for status in ["open", "pending", "resolved"]}
    severity_counts = {severity: sum(1 for q in queries if q["severity"] == severity) for severity in ["critical", "major", "minor"]}
    site_counts = {site: sum(1 for q in queries if q["site_id"] == site) for site in sites}
...
async def get_all_queries(
    ...
    query_generator = await _initialize_agent_with_fallback(QueryGenerator, "Query Generator")
...
def _generate_sdv_verification_contexts(subjects: list, sites_data: list, limit: int = 20):
    ...
    for subject_id in subjects[:limit]:
        ...
        source_data = {
            "vital_signs": {
                "systolic_bp": edc_data["vital_signs"]["systolic_bp"] + (2 if subject_hash % 5 == 0 else 0),
                "diastolic_bp": edc_data["vital_signs"]["diastolic_bp"] + (-2 if subject_hash % 7 == 0 else 0),
                "heart_rate": edc_data["vital_signs"]["heart_rate"] + (1 if subject_hash % 3 == 0 else 0)
            },
            "laboratory": {
                "hemoglobin": edc_data["laboratory"]["hemoglobin"] + (-0.2 if subject_hash % 4 == 0 else 0),
                "creatinine": edc_data["laboratory"]["creatinine"] + (0.05 if subject_hash % 6 == 0 else 0),
                "bnp": edc_data["laboratory"]["bnp"] + (-2 if subject_hash % 8 == 0 else 0)
            },
...
async def _process_sdv_sessions(verification_contexts: list, data_verifier):
        ...
        try:
            if data_verifier:
                ...
                for category in ["vital_signs", "laboratory"]:
                    if category in context["edc_data"] and category in context["source_data"]:
            ...
            critical_findings = sum(1 for d in discrepancies if d.get("severity") == "critical")
            ...
            return {
                ...
                "source_documents_reviewed": ["medical_history_form", "vital_signs_log", "laboratory_results", "adverse_events_log"],
                "next_monitoring_date": "2025-02-09"
            }
        except Exception as e:
            print(f"SDV session processing failed for {context.get('subject_id', 'unknown')}: {e}")
    ...
    tasks = [process_single_session(i, context) for i, context in enumerate(verification_contexts)]
...
def _calculate_site_progress(sites_data: list, subjects: list, sdv_sessions: list):
    ...
    for site_data in sites_data:
        site_subjects = [s for s in subjects if hash(s) % 3 == int(site_data["site_id"][-1]) - 1][:17]
        site_sessions = [s for s in sdv_sessions if s["site_id"] == site_data["site_id"]]
        ...
        progress = {
            ...
            "verification_percentage": round((verified_count / len(site_subjects)) * 100, 1) if site_subjects else 0,
            "pending_subjects": len(site_subjects) - verified_count,
            "last_visit": "2025-01-09",
            "monitor_assigned": site_data["monitor"],
            "risk_level": "low" if verified_count >= len(site_subjects) * 0.8 else "medium"
        }
...
async def get_sdv_sessions(
    ...
    subjects = test_service.get_available_subjects()
    sites_data = [
        {"site_id": "SITE_001", "site_name": "Metropolitan Medical Center", "monitor": "Dr. Michael Chen"},
        {"site_id": "SITE_002", "site_name": "Regional Heart Institute", "monitor": "Lisa Rodriguez"},
        {"site_id": "SITE_003", "site_name": "University Cardiology Center", "monitor": "Dr. Sarah Kim"}
    ]
    
    # Generate verification contexts efficiently
    verification_contexts = _generate_sdv_verification_contexts(subjects, sites_data, 20)
...
async def get_protocol_deviations(
    ...
    for subject_id in subjects[:10]:  # Check first 10 subjects
        ...
        protocol_context = {
            ...
            "subject_age": 45 if hash(subject_id) % 20 != 0 else 17,  # Some subjects under 18
            "visit_dates": {
                "baseline": "2024-12-01",
                "week_4": "2024-12-29" if hash(subject_id) % 10 == 0 else "2024-12-22",  # Some outside window
                "week_8": "2025-01-26"
            },
    ...
    for context in protocol_contexts:
        ...
        detection_result = await deviation_detector.detect_protocol_deviations_ai(protocol_data, subject_data)
        ...
        for deviation in detected_deviations:
            formatted_deviation = {
                ...
                "status": "under_review" if deviation.get("severity") == "major" else "resolved",
                "detected_date": "2025-01-09T10:00:00",
                "reported_date": "2025-01-09T12:00:00",
                "description": deviation.get("description", "Protocol deviation detected"),
                "protocol_section": deviation.get("protocol_section", "Unknown"),
                "impact_assessment": deviation.get("impact_assessment", "low_regulatory_risk"),
                "capa_required": deviation.get("severity") == "major",
                "capa_due_date": "2025-01-23T10:00:00" if deviation.get("severity") == "major" else None,
                "investigator": f"Dr. {context['site_id'].replace('SITE_', 'Site ')} Investigator",
                "root_cause": deviation.get("root_cause", "system_error"),
                "corrective_action": deviation.get("corrective_action", "Standard correction applied"),
                "preventive_action": deviation.get("preventive_action", "Process improvement implemented")
            }
    ...
    compliance_rate = ((total_subjects_checked - len(all_deviations)) / total_subjects_checked) * 100 if total_subjects_checked > 0 else 100
    ...
    compliance_metrics = {
        ...
        "risk_score": "high" if active_deviations > 3 else "medium" if active_deviations > 1 else "low",
        "trend": "improving" if resolved_deviations > active_deviations else "stable"
    }
    
    return ProtocolDeviationsResponse(deviations=all_deviations, compliance_metrics=compliance_metrics)
...
async def create_sdv_session(
    ...
    verification_context = {
        ...
        "verification_scope": ["vital_signs", "laboratory", "demographics", "adverse_events"]
    }
    
    # Use Data Verifier agent to set up the session
    session_setup_result = await data_verifier.setup_verification_session(verification_context)
    ...
    return {
        ...
        "verification_scope": session_metadata.get("verification_scope", ["vital_signs", "laboratory"]),
...
async def get_protocol_monitoring(
    ...
    monitoring_result = await portfolio_manager.orchestrate_monitoring_workflow(monitoring_request)
    ...
    if monitoring_result.get("success"):
        ...
        monitoring_schedule = [{
            ...
            "next_visit_date": (datetime.now() + timedelta(days=10)).strftime("%Y-%m-%d"),
...
async def get_dashboard_analytics(
    ...
    ai_analytics_result = await analytics_agent.generate_analytics_insights_ai(trial_data)
    
    # Check if AI analysis was successful
    if ai_analytics_result.get("ai_powered") and "analytics_insights" in ai_analytics_result:
        ...
        recent_activities = [
            {
                ...
                "type": "ai_insight" if i == 0 else ["subject_enrolled", "query_resolved", "deviation_detected"][i % 3],
                "subject_id": f"CARD{(len(subjects) - i):03d}" if i > 0 else "N/A",
                "site_id": sites[i % 3],
                "timestamp": (datetime.now() - timedelta(hours=i*2)).isoformat(),
                "description": ai_analytics_result.get("executive_summary", "AI analysis completed") if i == 0 
                              else ["New subject enrolled", "Query resolved", "Protocol deviation detected"][i % 3],
                "performed_by": "AI Analytics" if i == 0 else f"Dr. {['Smith', 'Johnson', 'Davis'][i % 3]}"
            }
        ...
        analytics_result = await analytics_agent.generate_dashboard_analytics(analytics_request)
