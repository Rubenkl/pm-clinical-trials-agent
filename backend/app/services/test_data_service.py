...
from tests.test_data.synthetic_data_generator import STUDY_PRESETS, generate_test_study
...
class TestDataService:
    ...
    def _initialize_test_data(self):
        ...
        try:
            study_preset = getattr(
                self.settings, "test_data_preset", "cardiology_phase2"
            )
    ...
    def _build_lookup_cache(self):
        ...
        self.test_data_cache["subjects"] = {
            subject["subject_id"]: subject for subject in self.current_study["subjects"]
        }

        # Site lookup cache
        self.test_data_cache["sites"] = {
            site["site_id"]: site for site in self.current_study["sites"]
        }
    ...
    async def get_subject_data(
        self, subject_id: str, data_source: str = "edc"
        ...
        """
        if (
            not self.settings.use_test_data
            or subject_id not in self.test_data_cache["subjects"]
        ):
            return None
    ...
    async def get_visit_data(
        self, subject_id: str, visit_name: str, data_source: str = "edc"
    ) -> Optional[Dict[str, Any]]:
    ...
    async def get_discrepancies(
        self, subject_id: str, visit_name: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        if visit_name:
            ...
            if key in self.test_data_cache["visit_data"]:
                discrepancies.extend(
                    self.test_data_cache["visit_data"][key]["discrepancies"]
                )
        else:
    ...
    async def get_subjects_with_discrepancies(
        ...
        for subject in self.current_study["subjects"]:
            ...
            for visit in subject["visits"]:
                if severity:
                    visit_discrepancies = [
                        d for d in visit["discrepancies"] if d["severity"] == severity
                    ]
                else:
    ...
    async def get_site_performance_data(self) -> List[Dict[str, Any]]:
        ...
        for site_id, site_info in self.test_data_cache["sites"].items():
            ...
            site_subjects = [
                s for s in self.current_study["subjects"] if s["site_id"] == site_id
            ]
            ...
            total_discrepancies = sum(
                s["data_quality"]["discrepant_data_points"] for s in site_subjects
            )
            total_critical = sum(
                s["data_quality"]["critical_findings"] for s in site_subjects
            )
            ...
            site_performance.append(
                {
                    ...
                    "metrics": {
                        ...
                        "discrepancy_rate": total_discrepancies
                        / max(
                            total_subjects * 50, 1
                        ),  # Assume 50 data points per subject
                    },
                }
            )
...
async def get_test_data_for_agents(
    ...
    return {
        ...
        "example_subject_data": (
            await test_data_service.get_subject_data(
                test_data_service.get_available_subjects()[0], "both"
            )
            if test_data_service.get_available_subjects()
            else None
        ),
    }
