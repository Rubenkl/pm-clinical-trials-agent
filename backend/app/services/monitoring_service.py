...
class ClinicalMonitoringService:
    ...
    async def _perform_critical_checks(self):
        ...
        try:
            ...
            result = await portfolio_manager.orchestrate_query_workflow(
                critical_query_request
            )
            ...
            safety_result = await portfolio_manager.orchestrate_query_workflow(
                safety_request
            )
    ...
    async def _perform_deviation_checks(self):
        ...
        try:
            ...
            result = await portfolio_manager.orchestrate_deviation_workflow(
                deviation_request
            )
            if result.get("success"):
    ...
    async def _generate_alert(self, alert_data: Dict[str, Any]):
        ...
        logger.warning(
            f"Monitoring alert generated: {alert['alert_type']} - {alert.get('severity', 'medium')}"
        )
    ...
    def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_checks": {
                "critical": (
                    self.state.last_critical_check.isoformat()
                    if self.state.last_critical_check
                    else None
                ),
                "routine": (
                    self.state.last_routine_check.isoformat()
                    if self.state.last_routine_check
                    else None
                ),
                "sdv": (
                    self.state.last_sdv_check.isoformat()
                    if self.state.last_sdv_check
                    else None
                ),
            },
            "statistics": {
