...
from app.agents.query_tracker import QueryStatus, QueryTracker, TrackedQuery
...
class TestQueryTracker:
    ...
    async def test_generate_follow_up_message(self, query_tracker, sample_query):
        ...
        follow_up = await query_tracker.generate_follow_up(
            sample_query["query_id"], escalation_level=1
        )
    ...
    async def test_bulk_status_update(self, query_tracker):
        ...
        # Bulk update
        result = await query_tracker.bulk_update_status(query_ids, QueryStatus.RESOLVED)

        assert result["success"] is True
        assert result["updated_count"] == 3
        assert all(
            q.status == QueryStatus.RESOLVED
            for q in query_tracker.tracked_queries.values()
        )
    ...
    async def test_query_history(self, query_tracker, sample_query):
        ...
        await query_tracker.add_history_event(
            sample_query["query_id"], "reminder_sent", "First reminder sent to site"
        )

        await query_tracker.add_history_event(
            sample_query["query_id"], "site_responded", "Site acknowledged receipt"
        )
    ...
    async def test_priority_based_follow_up(self, query_tracker):
        ...
        critical_follow_up = next(
            (f for f in follow_ups if f["query_id"] == "Q_CRIT"), None
        )
