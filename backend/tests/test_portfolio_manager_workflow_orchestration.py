...
class TestPortfolioManagerWorkflowOrchestration:
    ...
    async def test_orchestrate_query_analysis_workflow(
        self, portfolio_manager, query_workflow_request
    ):
    ...
    async def test_orchestrate_sdv_workflow(
        self, portfolio_manager, sdv_workflow_request
    ):
        """Test Portfolio Manager orchestrates SDV workflow returning structured JSON"""
        # This test will FAIL initially (RED phase)
        result = await portfolio_manager.orchestrate_structured_workflow(
            sdv_workflow_request
        )
    ...
    async def test_orchestrate_deviation_detection_workflow(
        self, portfolio_manager, deviation_workflow_request
    ):
    ...
    async def test_human_readable_field_generation(
        self, portfolio_manager, query_workflow_request
    ):
        ...
        assert any(
            term in summary
            for term in ["hemoglobin", "anemia", "critical", "severe", "clinical"]
        )
        ...
        assert (
            "g/dL" in interpretation or "mmHg" in interpretation
        )  # Units for medical values
    ...
    async def test_agent_specialization_routing(self, portfolio_manager):
        ...
        workflows = [
            ...
            (
                ...
                "query_analyzer",
            ),  # Primary agent for complex workflows
        ]
...
class TestPortfolioManagerAgentIntegration:
    ...
    async def test_query_analyzer_integration(self, portfolio_manager):
        ...
        )  # Should detect critical hemoglobin
        ...
        assert any(
            term in clinical_interp.lower() for term in ["severe", "critical", "anemia"]
        )
...
class TestPortfolioManagerResponseStructure:
    ...
    async def test_frontend_compatibility(self, portfolio_manager):
        ...
        if (
            ...
            and "recommendations" in response_data["ai_analysis"]
        ):
