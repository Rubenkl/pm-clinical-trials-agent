...
class TestQueryAnalyzerAI:
    ...
    async def test_analyze_clinical_data_ai(self):
        ...
        with patch(
            "app.agents.query_analyzer.Runner.run", new_callable=AsyncMock
        ) as mock_run:
...
class TestDeviationDetectorAI:
    ...
    async def test_detect_protocol_deviations_ai(self):
        ...
        with patch(
            "app.agents.deviation_detector.Runner.run", new_callable=AsyncMock
        ) as mock_run:
            mock_run.return_value = mock_ai_response

            # This method needs to be implemented
            result = await detector.detect_protocol_deviations_ai(
        ...
        assert (
            "QT prolongation"
            in deviations_by_type["prohibited_medication"]["clinical_impact"]
        )
...
class TestQueryTrackerAI:
    ...
    async def test_track_query_lifecycle_ai(self):
        ...
        with patch(
            "app.agents.query_tracker.Runner.run", new_callable=AsyncMock
        ) as mock_run:
            mock_run.return_value = mock_ai_response
        ...
        assert (
            "pattern detected"
            in result["lifecycle_analysis"]["escalation_reasons"][2].lower()
        )

        # Verify predictive capabilities
        assert "predictive_analysis" in result["lifecycle_analysis"]
        assert (
            result["lifecycle_analysis"]["predictive_analysis"]["risk_of_data_loss"]
            > 0.7
        )
...
class TestAnalyticsAgentAI:
    ...
    async def test_generate_analytics_insights_ai(self):
        ...
        with patch(
            "app.agents.analytics_agent.Runner.run", new_callable=AsyncMock
        ) as mock_run:
            mock_run.return_value = mock_ai_response
        ...
        assert (
            result["analytics_insights"]["predictive_insights"][
                "regulatory_submission_risk"
            ]
            == "medium-high"
        )
