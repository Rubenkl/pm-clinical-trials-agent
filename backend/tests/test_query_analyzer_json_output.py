...
from app.api.models.structured_responses import QueryAnalyzerResponse, SeverityLevel, QueryStatus
...
class TestQueryAnalyzerJSONOutput:
    ...
    async def test_query_analyzer_json_output_structure(self, query_analyzer, critical_hemoglobin_data):
    ...
    async def test_critical_hemoglobin_analysis(self, query_analyzer, critical_hemoglobin_data):
        ...
        assert any("transfusion" in rec.lower() or "hematology" in rec.lower() for rec in ai_analysis["recommendations"])
        ...
        assert any(term in clinical_interp.lower() for term in ["severe anemia", "anemia", "critical"])

    async def test_elevated_blood_pressure_analysis(self, query_analyzer, elevated_bp_data):
        ...
        assert any(term in finding["interpretation"].lower() for term in ["hypertension", "hypertensive", "elevated", "crisis"])
        
        # AI analysis should include appropriate recommendations
        recommendations = result["ai_analysis"]["recommendations"]
        assert any("cardiology" in rec.lower() or "antihypertensive" in rec.lower() for rec in recommendations)
        
        # Human-readable summary should include blood pressure context
        summary = result["human_readable_summary"].lower()
        assert any(term in summary for term in ["blood pressure", "bp", "hypertension", "185"])
    ...
    async def test_normal_value_handling(self, query_analyzer, normal_value_data):
        ...
        assert result["severity"] in [SeverityLevel.INFO.value, SeverityLevel.MINOR.value]
        
        # Should indicate no query needed in recommendations
        ai_analysis = result["ai_analysis"]
        assert "no action" in ai_analysis["interpretation"].lower() or "normal" in ai_analysis["interpretation"].lower()
    ...
    async def test_query_analyzer_response_validation(self, query_analyzer, critical_hemoglobin_data):
    ...
    async def test_medical_intelligence_integration(self, query_analyzer):
        ...
        test_cases = [
            {
                "data": {
                    "subject_id": "SUBJ004", "site_id": "SITE01", "visit": "Week 1",
                    "field_name": "creatinine", "field_value": "3.2", "normal_range": "0.6-1.2 mg/dL"
                },
            ...
            {
                "data": {
                    "subject_id": "SUBJ005", "site_id": "SITE02", "visit": "Week 2", 
                    "field_name": "platelet_count", "field_value": "45000", "normal_range": "150000-450000 /μL"
                },
                "expected_severity": SeverityLevel.CRITICAL.value,
        ...
        for test_case in test_cases:
            ...
            assert any(len(rec) > 10 for rec in recommendations)  # Meaningful recommendations

    async def test_human_readable_field_quality(self, query_analyzer, critical_hemoglobin_data):
        ...
        assert any(unit in interpretation for unit in ["g/dL", "mg/dL", "mmHg", "/μL"])  # Medical units
        ...
        assert any(word in rec_summary.lower() for word in ["recommend", "suggest", "consider", "should"])
        ...
        test_scenarios = [
            {
                "data": {
                    ...
                    "field_name": "hemoglobin", "field_value": "7.0", "normal_range": "12.0-15.5 g/dL"
                },
            ...
            },
            {
                "data": {
                    ...
                    "field_name": "glucose", "field_value": "102", "normal_range": "70-100 mg/dL"
                },
        ...
        batch_data = [
            {
                ...
                "field_name": "hemoglobin", "field_value": "8.2", "normal_range": "12.0-15.5 g/dL"
            },
            {
                "subject_id": "SUBJ009", "site_id": "SITE01", "visit": "Week 1",
                "field_name": "systolic_bp", "field_value": "165", "normal_range": "<140 mmHg"
            },
            {
                "subject_id": "SUBJ010", "site_id": "SITE02", "visit": "Week 1",
                "field_name": "glucose", "field_value": "95", "normal_range": "70-100 mg/dL"
            }
        ]
