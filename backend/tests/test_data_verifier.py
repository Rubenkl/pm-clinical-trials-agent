...
class TestDataVerifierCore:
    ...
    def setup_method(self):
        ...
        self.source_data = {
            ...
            "adverse_events": [{"term": "Headache", "severity": "Mild"}],  # Missing in EDC
            "concomitant_meds": ["Aspirin 81mg"]
        }
    ...
    def test_data_verifier_initialization(self):
        ...
        assert "verifying and validating clinical trial data" in self.data_verifier.description
    ...
    async def test_cross_system_verification_basic(self):
        ...
        mock_response = AgentResponse(
            ...
            content=json.dumps({
                ...
                "matches": ["vital_signs.diastolic_bp", "vital_signs.heart_rate", "concomitant_meds"],
        ...
        with patch.object(self.data_verifier, 'process_message', new_callable=AsyncMock) as mock_process:
            ...
            result = await self.data_verifier.cross_system_verification(self.edc_data, self.source_data)
            ...
            bp_discrepancy = next(d for d in result.discrepancies if d.field_name == "vital_signs.systolic_bp")
    ...
    async def test_critical_data_identification(self):
        ...
        critical_data = {
            ...
            "eligibility": {
                "inclusion_criteria": False  # Critical protocol violation
            }
        }
        ...
        with patch.object(self.data_verifier, 'process_message', new_callable=AsyncMock) as mock_process:
            ...
            risk_assessment = await self.data_verifier.assess_critical_data(critical_data)
            ...
            ae_field = next(f for f in risk_assessment.critical_fields 
    ...
    async def test_discrepancy_pattern_detection(self):
        ...
        with patch.object(self.data_verifier, 'process_message', new_callable=AsyncMock) as mock_process:
            ...
            patterns = await self.data_verifier.detect_discrepancy_patterns(historical_discrepancies)
    ...
    async def test_audit_trail_generation(self):
        ...
        with patch.object(self.data_verifier, 'process_message', new_callable=AsyncMock) as mock_process:
            ...
            audit_trail = await self.data_verifier.generate_audit_trail(verification_data)
...
class TestDataVerifierEdgeCases:
    ...
    async def test_verification_with_missing_fields(self):
        ...
        with patch.object(self.data_verifier, 'process_message', new_callable=AsyncMock) as mock_process:
            ...
            result = await self.data_verifier.cross_system_verification(incomplete_edc, incomplete_source)
    ...
    async def test_verification_failure_handling(self):
        ...
        with patch.object(self.data_verifier, 'process_message', new_callable=AsyncMock) as mock_process:
            ...
            with pytest.raises(Exception, match="Verification failed: OpenAI API rate limit exceeded"):
    ...
    async def test_invalid_json_response_handling(self):
        ...
        with patch.object(self.data_verifier, 'process_message', new_callable=AsyncMock) as mock_process:
            ...
            with pytest.raises(Exception, match="Failed to parse verification response"):
                await self.data_verifier.cross_system_verification({}, {})
...
class TestDataVerifierPerformance:
    ...
    async def test_batch_verification_performance(self):
        ...
        for i in range(5):
            ...
            source_data = {"subject_id": f"S{i:03d}", "visit": "V1", "data": f"value_{i}_source"}
        ...
        mock_response = AgentResponse(
            ...
            content=json.dumps({
                "batch_results": [
                    {"subject_id": f"S{i:03d}", "discrepancies_found": 1, "accuracy": 0.8}
                    for i in range(5)
                ],
...
class TestDataVerifierIntegration:
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_verification_workflow(self, sample_edc_data, sample_source_data):
        ...
        with patch.object(self.data_verifier, 'process_message', new_callable=AsyncMock) as mock_process:
            ...
            result = await self.data_verifier.complete_sdv_verification(sample_edc_data, sample_source_data)
            ...
            critical_discrepancies = [d for d in result.discrepancies if d.severity == "critical"]
            assert len(critical_discrepancies) == 0
            
            # Verify recommended actions are provided
            assert len(result.recommended_actions) == 3
            assert any("query" in action.lower() for action in result.recommended_actions)