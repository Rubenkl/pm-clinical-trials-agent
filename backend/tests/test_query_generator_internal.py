...
class TestQueryGeneratorInternal:
    ...
    def clinical_analysis_result(self):
        ...
        return {
            ...
            "ai_analysis": {
                ...
                "recommendations": ["Immediate medical review", "Check for bleeding sources"]
            }
        }
    ...
    async def test_query_generator_internal_integration(self, query_generator, clinical_analysis_result):
        ...
        assert clinical_analysis_result["query_id"] in result["query_metadata"]["source_analysis_id"]
        ...
        assert any(term in query_text.lower() for term in ["severe", "anemia", "critical"])

    async def test_portfolio_manager_query_generator_handoff(self, portfolio_manager, clinical_analysis_result):
    ...
    async def test_query_generator_discrepancy_integration(self, query_generator, verification_result):
        ...
        assert any(term in query_text.lower() for term in ["discrepancy", "difference", "mismatch"])
    ...
    async def test_query_generator_workflow_chaining(self, query_generator):
        ...
        workflow_context = {
            ...
            "input_data": {
                "analysis_result": {
                    "severity": "major",
                    "clinical_findings": [{"parameter": "glucose", "value": "250 mg/dL"}]
                },
                "verification_result": {
                    "discrepancies": [{"field": "glucose", "severity": "major"}]
                }
            },
            "agent_chain": ["query_analyzer", "data_verifier", "query_generator", "query_tracker"]
        }
    ...
    async def test_query_generator_compliance_integration(self, query_generator):
        ...
        query_text = result["query_text"]
        assert any(term in query_text.lower() for term in ["regulatory", "compliance", "ich-gcp"])
...
class TestQueryTrackerInternal:
    ...
    async def test_query_tracker_workflow_integration(self, query_tracker, generated_query):
    ...
    async def test_portfolio_manager_query_tracker_handoff(self, portfolio_manager, generated_query):
    ...
    async def test_query_tracker_sla_workflow(self, query_tracker):
        ...
        assert "4" in result["response_deadline"] or "hour" in result["response_deadline"]
