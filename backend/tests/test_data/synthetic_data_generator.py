...
class SyntheticDataGenerator:
    ...
    def __init__(self, config: StudyConfiguration):
        ...
        self.therapeutic_templates = {
            "cardiology": {
                "vital_signs": {"systolic_bp": (110, 180), "diastolic_bp": (70, 110), "heart_rate": (60, 100)},
                "laboratory": {"troponin": (0.01, 0.15), "bnp": (50, 400), "creatinine": (0.8, 2.0)},
                "imaging": {"lvef": (40, 70), "wall_motion": ["normal", "mild_hypokinesis", "moderate_hypokinesis"]},
                "adverse_events": ["chest_pain", "dyspnea", "palpitations", "syncope"]
            },
            "oncology": {
                "vital_signs": {"systolic_bp": (100, 160), "diastolic_bp": (60, 100), "heart_rate": (70, 120)},
                "laboratory": {"hemoglobin": (8.0, 15.0), "platelets": (50, 400), "alt": (10, 300)},
                "tumor_assessment": {"target_lesions": (1, 5), "sum_diameters": (10, 150)},
                "adverse_events": ["fatigue", "nausea", "neutropenia", "thrombocytopenia"]
            }
        }
    ...
    def _generate_subjects(self) -> List[Dict[str, Any]]:
        ...
        subjects = []
        template = self.therapeutic_templates.get(self.config.therapeutic_area, self.therapeutic_templates["cardiology"])
    ...
    def _generate_visits(self, subject_id: str, template: Dict) -> List[Dict[str, Any]]:
        ...
        visits = []
        visit_schedule = ["Screening", "Baseline", "Week_4", "Week_8", "Week_12", "End_of_Study"]
    ...
    def _generate_visit_data(self, template: Dict) -> Dict[str, Any]:
        ...
        for key, value in template.items():
            if key not in ["vital_signs", "laboratory", "adverse_events"]:
                if isinstance(value, dict) and all(isinstance(v, tuple) for v in value.values()):
                    # Numeric parameters
                    section = {}
                    ...
                    for param, options in value.items():
                        if isinstance(options, list):
                            ...
                            section[param] = round(random.uniform(options[0], options[1]), 1)
                    data[key] = section
        ...
        if random.random() < 0.3:  # 30% chance of AE
            ...
            data["adverse_events"] = [{
                ...
                "outcome": random.choice(["Ongoing", "Recovered", "Recovered with sequelae"])
            }]
        else:
            data["adverse_events"] = []
    ...
    def _generate_source_data_with_discrepancies(self, edc_data: Dict, template: Dict) -> Dict[str, Any]:
                ...
                if isinstance(values, dict):
                    for param, value in values.items():
                        if isinstance(value, (int, float)) and random.random() < 0.3:
                            ...
                            values[param] = round(value + random.choice([-variance, variance]), 2)
    ...
    def _identify_discrepancies(self, edc_data: Dict, source_data: Dict) -> List[Dict[str, Any]]:
            ...
            for key in set(list(edc_dict.keys()) + list(source_dict.keys())):
                ...
                if key not in edc_dict:
                    discrepancies.append({
                        ...
                        "severity": "major" if "adverse_events" in current_path else "minor"
                    })
                ...
                elif isinstance(edc_dict[key], dict) and isinstance(source_dict[key], dict):
                    ...
                    discrepancies.append({
                        ...
                        "edc_value": edc_dict[key],
                        "source_value": source_dict[key],
                        "severity": self._assess_discrepancy_severity(current_path, edc_dict[key], source_dict[key])
                    })
        
        compare_nested_dicts(edc_data, source_data)
        return discrepancies
    
    def _assess_discrepancy_severity(self, field: str, edc_value: Any, source_value: Any) -> str:
    ...
    def _generate_queries_for_discrepancies(self, discrepancies: List[Dict], subject_id: str, visit: str) -> List[Dict[str, Any]]:
        ...
        for i, discrepancy in enumerate(discrepancies):
            query = {
                ...
                "response_required_by": (datetime.now() + timedelta(days=7)).strftime("%Y-%m-%d")
            }
    ...
    def _calculate_subject_data_quality(self, subject: Dict) -> Dict[str, int]:
        ...
        for visit in subject["visits"]:
            ...
            critical_findings += len([d for d in visit["discrepancies"] if d["severity"] == "critical"])
...
if __name__ == "__main__":
    ...
    print(f"Expected discrepancy rate: {study_data['discrepancy_summary']['expected_discrepancy_rate']}")