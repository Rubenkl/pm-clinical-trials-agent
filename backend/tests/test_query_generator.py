...
class TestQueryGenerator:
    ...
    @pytest.mark.asyncio
    async def test_generate_query_with_site_preferences(
        self, query_generator, sample_analysis
    ):
...
Thanks."""
        ...
        assert validation["valid"] is False
        assert any(
            "Missing standard phrases" in issue for issue in validation["issues"]
        )
    ...
    async def test_multi_language_support(self, query_generator):
        ...
        for lang in languages:
            result = await query_generator.generate_query(
                {"category": "missing_data", "subject_id": "TEST001"}, language=lang
            )
