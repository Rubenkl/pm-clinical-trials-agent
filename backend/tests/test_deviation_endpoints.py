...
class TestDeviationDetectionEndpoint:
    ...
    def test_deviation_detection_endpoint_exists(self):
        ...
        response = client.post("/api/v1/deviations/detect", json={
            "subject_id": "SUBJ001",
            "site_id": "SITE01",
            "visit": "Week 12",
    ...
    def test_deviation_detection_identifies_deviations(self):
        ...
        visit_deviation = next(
            (d for d in data["deviations"] if "visit_window" in d["category"]),
            None
        )
        ...
        fasting_deviation = next(
            (d for d in data["deviations"] if "fasting" in d["category"]),
            None
        )
...
class TestDeviationStatisticsEndpoint:
    ...
    def test_deviation_stats_returns_valid_structure(self):
        ...
        required_fields = [
            ...
            "minor_deviations", "resolved_deviations", "pending_deviations",
            "deviations_by_site", "deviations_by_category", "deviation_trends",
            "resolution_rate", "average_resolution_time"
        ]
...
class TestDeviationResolutionEndpoint:
    ...
    def test_deviation_resolution_endpoint_exists(self):
        ...
        response = client.post("/api/v1/deviations/DEV-001/resolve", json={
    ...
    def test_deviation_resolution_with_valid_data(self):
        ...
        response = client.post("/api/v1/deviations/DEV-001/resolve", json=resolution_data)
        assert response.status_code == 200
