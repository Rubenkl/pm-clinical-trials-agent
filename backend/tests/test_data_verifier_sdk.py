...
class TestDataVerifierSDK:
    ...
    def mock_openai_client(self):
        ...
        with patch('app.agents.data_verifier_sdk.OpenAI') as mock_client:
            ...
            mock_message.content = [Mock(text=Mock(value=json.dumps({
                ...
                "critical_findings": [],
                "recommendations": ["Review source document for hemoglobin value"]
            })))]
    ...
    def test_critical_fields_configuration(self, data_verifier):
        ...
        expected_critical_fields = [
            "hemoglobin", "blood_pressure", "heart_rate", "temperature",
            "adverse_events", "serious_adverse_events", "concomitant_medications"
        ]
        
        for field in expected_critical_fields:
            assert any(field in critical_field for critical_field in data_verifier.critical_fields)
    ...
    @pytest.mark.asyncio
    async def test_cross_system_verification(self, data_verifier, sample_edc_data, sample_source_data):
        """Test cross-system data verification."""
        result = await data_verifier.cross_system_verification(sample_edc_data, sample_source_data)
    ...
    @pytest.mark.asyncio
    async def test_complete_sdv_verification(self, data_verifier, sample_edc_data, sample_source_data):
        """Test complete SDV (Source Data Verification) process."""
        result = await data_verifier.complete_sdv_verification(sample_edc_data, sample_source_data)
    ...
    def test_discrepancy_severity_assessment(self, data_verifier):
        ...
        assert critical_severity in [DiscrepancySeverity.CRITICAL, DiscrepancySeverity.MAJOR]
    ...
    async def test_format_difference_detection(self, data_verifier):
        ...
        format_discrepancies = [
            ...
            if d.get("discrepancy_type") == "format_difference"
        ]
    ...
    async def test_unit_mismatch_detection(self, data_verifier):
        ...
        weight_issues = [
            d for d in discrepancies 
            if d.get("field_name") == "weight" and 
            d.get("discrepancy_type") in ["unit_mismatch", "value_mismatch"]
        ]
    ...
    async def test_error_handling(self, data_verifier):
        ...
        result = await data_verifier.cross_system_verification(invalid_edc, invalid_source)
    ...
    @pytest.mark.asyncio
    async def test_context_accumulation(self, data_verifier, sample_edc_data, sample_source_data):
        """Test context accumulation across multiple verifications."""
        # First verification
        result1 = await data_verifier.cross_system_verification(sample_edc_data, sample_source_data)
        ...
        result2 = await data_verifier.cross_system_verification(modified_edc, sample_source_data)
    ...
    async def test_regulatory_compliance_checking(self, data_verifier):
        ...
        assert "regulatory_compliance" in assessment or "critical_findings" in assessment
        assert isinstance(assessment, dict)
    
    @pytest.mark.asyncio
    async def test_statistical_analysis_integration(self, data_verifier, sample_batch_data):
        ...
        expected_stats = ["total_discrepancies", "discrepancy_rate", "critical_findings_count"]
        for stat in expected_stats:
            assert stat in stats or len(stats) > 0  # Flexible assertion for implementation variation