...
class TestDataVerifierJSONOutput:
    ...
    def perfect_match_data(self):
        ...
        return {
            ...
            "site_id": "SITE02",
            "visit": "Week 4",
            "edc_data": {"hemoglobin": "13.2", "glucose": "88", "weight": "68.5"},
            "source_data": {"hemoglobin": "13.2", "glucose": "88", "weight": "68.5"},
        }

    async def test_data_verifier_json_output_structure(
        self, data_verifier, major_discrepancy_data
    ):
    ...
    async def test_major_discrepancy_detection(
        self, data_verifier, major_discrepancy_data
    ):
        ...
        hemoglobin_discrepancy = next(
            (d for d in result["discrepancies"] if d["field"] == "hemoglobin"), None
        )
        ...
        assert hemoglobin_discrepancy["severity"] in [
        ...
        ae_discrepancy = next(
            (d for d in result["discrepancies"] if d["field"] == "adverse_events"), None
        )
        ...
        assert any(
            term in verification_summary.lower()
            for term in ["mismatch", "discrepancy", "difference"]
        )

    async def test_minor_discrepancy_handling(
        self, data_verifier, minor_discrepancy_data
    ):
        ...
        assert any(
            term in summary for term in ["minor", "acceptable", "within tolerance"]
        )
    ...
    async def test_perfect_match_verification(self, data_verifier, perfect_match_data):
        ...
        assert any(
            term in summary
            for term in ["complete", "verified", "no discrepancies", "match"]
        )

    async def test_data_verifier_response_validation(
        self, data_verifier, major_discrepancy_data
    ):
    ...
    async def test_medical_discrepancy_intelligence(self, data_verifier):
        ...
        critical_discrepancies = [
            d
            for d in result["discrepancies"]
            if d["severity"] == SeverityLevel.CRITICAL.value
        ]
    ...
    async def test_human_readable_field_quality(
        ...
        assert any(
            word in verification_summary.lower()
            for word in ["verified", "discrepancy", "mismatch", "reviewed"]
        )
        ...
        assert any(
            field in findings
            for field in ["hemoglobin", "adverse_events", "systolic_bp"]
        )
...
class TestDataVerifierIntegration:
    ...
    async def test_structured_endpoint_compatibility(self, data_verifier):
        ...
        assert result["match_score"] < 0.8
        bp_discrepancy = next(
            (d for d in result["discrepancies"] if "bp" in d["field"].lower()), None
        )
            ...
            large_dataset.append(
                {
                    ...
                    "source_data": {
                        "hemoglobin": str(12.0 + (i * 0.1) + (0.5 if i % 2 == 0 else 0))
                    },
                }
            )
