...
class TestDeviationDetectorAgent:
    ...
    async def test_detect_protocol_deviations_basic(self, deviation_detector, protocol_violation_data):
        """Test basic protocol deviation detection with JSON output"""
        # This test will FAIL initially (RED phase)
        result = await deviation_detector.detect_protocol_deviations(protocol_violation_data)
        ...
        medication_violation = next(
            (d for d in deviations if d["category"] == "prohibited_medication"), 
            None
        )
    ...
    async def test_compliant_data_no_deviations(self, deviation_detector, compliant_data):
        ...
        assert "no" in result["human_readable_summary"].lower() and "deviations" in result["human_readable_summary"].lower()
    ...
    async def test_deviation_severity_classification(self, deviation_detector):
        ...
        critical_data = {
            ...
            "subject_id": "SUBJ005", "site_id": "SITE01", "visit": "Week 1"
        }
        ...
        batch_data = [
            {
                ...
                "subject_id": "SUBJ008", "site_id": "SITE01", "visit": "Week 1"
            },
            {
                "protocol_data": {"required_visit_window": "±3 days"},
                "actual_data": {"visit_date": "2025-01-15", "scheduled_date": "2025-01-08"},
    ...
    async def test_deviation_context_preservation(self, deviation_detector):
        ...
        data1 = {
            "protocol_data": {"prohibited_medications": ["aspirin"]},
            "actual_data": {"concomitant_medications": ["aspirin"]},
            "subject_id": "SUBJ011", "site_id": "SITE01", "visit": "Week 1"
        }
        ...
        data2 = {
            "protocol_data": {"prohibited_medications": ["warfarin"]},
            "actual_data": {"concomitant_medications": ["warfarin"]},
            "subject_id": "SUBJ012", "site_id": "SITE01", "visit": "Week 1"
        }
    ...
    async def test_deviation_response_format_consistency(self, deviation_detector, protocol_violation_data):
        ...
        required_fields = [
            "success", "deviations", "total_deviations_found", "compliance_status",
            "subject_id", "site_id", "visit", "detection_date", "recommendations",
            "corrective_actions_required", "human_readable_summary", "deviation_summary",
            "compliance_assessment", "execution_time", "agent_id"
        ]
        ...
        for deviation in result["deviations"]:
            deviation_fields = [
                "category", "severity", "protocol_requirement", "actual_value",
                "impact_level", "corrective_action_required", "deviation_description", "confidence"
            ]
    ...
    async def test_deviation_medical_context_integration(self, deviation_detector):
        ...
        # This test will FAIL initially (RED phase)
        result = await deviation_detector.detect_protocol_deviations(medical_deviation_data)
        ...
        medical_deviations = [d for d in result["deviations"] if d["category"] in ["vital_signs", "laboratory_value"]]
...
class TestDeviationDetectorIntegration:
    ...
    async def test_deviation_detector_portfolio_manager_integration(self, deviation_detector):
        ...
        result = await portfolio_manager.orchestrate_structured_workflow(deviation_request)
    ...
    async def test_deviation_detector_structured_endpoint_compatibility(self, deviation_detector):
...
class TestDeviationDetectorPerformance:
    ...
    async def test_deviation_detection_performance(self, deviation_detector):
        ...
        for i in range(50):  # 50 subjects
            large_dataset.append({
                "protocol_data": {"prohibited_medications": ["aspirin", "warfarin"]},
                "actual_data": {"concomitant_medications": ["aspirin" if i % 2 == 0 else "metformin"]},
    ...
    async def test_deviation_detector_minimal_memory_usage(self, deviation_detector):
        ...
        context_attrs = [attr for attr in dir(deviation_detector.context) if not attr.startswith('_')]
        deviation_related = [attr for attr in context_attrs if 'deviation' in attr.lower() or 'compliance' in attr.lower()]
        ...
        pydantic_methods = ['construct', 'from_orm', 'model_computed_fields', 'model_config', 'model_construct', 
                           'model_copy', 'model_dump', 'model_dump_json', 'model_extra', 'model_fields', 
                           'model_fields_set', 'model_json_schema', 'model_parametrized_name', 'model_post_init', 
                           'model_rebuild', 'model_validate', 'model_validate_json', 'model_validate_strings', 
                           'parse_file', 'parse_obj', 'parse_raw', 'schema', 'schema_json', 'update_forward_refs', 'validate']
        non_deviation_attrs = [attr for attr in context_attrs if attr not in deviation_related and attr not in ['dict', 'json', 'copy', 'update'] and attr not in pydantic_methods]
        assert len(non_deviation_attrs) <= 5  # Minimal non-deviation fields only (excluding Pydantic methods)