...
class TestCompleteWorkflowWithSyntheticData:
    ...
    async def test_discrepancy_detection_workflow(self, test_data_service):
        ...
        subjects_with_discrepancies = (
            await test_data_service.get_subjects_with_discrepancies()
        )
    ...
    async def test_critical_finding_identification(self, test_data_service):
        ...
        critical_subjects = await test_data_service.get_subjects_with_discrepancies(
            "critical"
        )

        for subject in critical_subjects:
            critical_discrepancies = [
                d for d in subject["discrepancies"] if d["severity"] == "critical"
            ]
            ...
            for discrepancy in critical_discrepancies:
                ...
                assert any(
                    keyword in field
                    for keyword in ["adverse_events", "eligibility", "death", "safety"]
                )
    ...
    async def test_workflow_data_consistency(self, test_data_service):
        ...
        visit_data_baseline = await test_data_service.get_visit_data(
            test_subject, "Baseline", "both"
        )
    ...
    async def test_mocked_agent_integration(self, mock_orchestrate, test_data_service):
        ...
        mock_orchestrate.return_value = (
            '{"success": true, "workflow_id": "WF_001", "result": "completed"}'
        )
    ...
    async def test_end_to_end_data_flow(self, test_data_service):
        ...
        subject_data = await test_data_service.get_subject_data(
            priority_subject, "both"
        )
        assert subject_data is not None
        ...
        print(
            f"✅ End-to-end test completed successfully for subject {priority_subject}"
        )
...
async def test_synthetic_data_generation():
    ...
    print(
        f"✅ Generated study with {len(subjects)} subjects and {len(subjects_with_discrepancies)} having discrepancies"
    )
