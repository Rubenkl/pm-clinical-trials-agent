...
class TestPortfolioManagerSDK:
    ...
    @pytest.mark.asyncio
    async def test_orchestrate_query_resolution(
        self, portfolio_manager, sample_workflow_request
    ):
    ...
    async def test_agent_coordination(self, portfolio_manager):
        ...
        coordination_result = await portfolio_manager.coordinate_agents(
            agents_to_coordinate
        )
    ...
    @pytest.mark.asyncio
    async def test_workflow_context_management(
        self, portfolio_manager, sample_workflow_request
    ):
        """Test workflow context creation and management."""
        # Start workflow
        workflow_result = await portfolio_manager.orchestrate_workflow(
    ...
    async def test_workflow_cancellation(
        ...
        cancel_result = await portfolio_manager.cancel_workflow(
            workflow_id, reason="User requested cancellation"
        )
    ...
    def test_workflow_context_class(self):
        ...
        assert (
            context.active_workflows["test123"]["status"] == WorkflowStatus.IN_PROGRESS
        )
