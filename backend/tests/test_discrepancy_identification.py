...
class TestDiscrepancyIdentification:
    ...
    def test_missing_data_identification(self):
        ...
        assert missing_edc.severity == DiscrepancySeverity.CRITICAL  # Adverse events are critical
    ...
    def test_format_difference_identification(self):
        ...
        discrepancy = self.verifier._identify_format_difference(
            ...
            edc_value="01/15/1985",
            source_value="1985-01-15"
        )
    ...
    def test_batch_discrepancy_identification(self):
        ...
        bp_discrepancy = next((d for d in discrepancies if "systolic_bp" in d.field_name), None)
        ...
        glucose_discrepancy = next((d for d in discrepancies if "glucose" in d.field_name), None)
        assert glucose_discrepancy is not None
        assert glucose_discrepancy.difference == 3
        
        # Check adverse events discrepancy
        ae_discrepancy = next((d for d in discrepancies if "adverse_events" in d.field_name), None)
        assert ae_discrepancy is not None
        assert ae_discrepancy.discrepancy_type == DiscrepancyType.MISSING_IN_EDC
        assert ae_discrepancy.severity == DiscrepancySeverity.CRITICAL  # Adverse events are critical
    ...
    def test_field_specific_pattern_detection(self):
        ...
        field_discrepancies = [
            ...
            {"field": "vital_signs.diastolic_bp", "difference": 1, "subject_id": "S001"},
        ]
    ...
    def test_discrepancy_clustering(self):
        ...
        bp_cluster = next((c for c in clusters if "vital_signs" in c["field_pattern"]), None)
