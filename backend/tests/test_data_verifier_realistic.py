...
class TestDataVerifierWithGroundTruth:
    ...
    def _create_mock_response(
        self, expected_discrepancies: List[Dict[str, Any]], overall_accuracy: float
    ) -> AgentResponse:
        ...
        for expected in expected_discrepancies:
            ...
            detection_prob = (
                0.85 if expected.get("expected_severity") == "critical" else 0.70
            )

            if (
                pytest.current_confidence_modifier * detection_prob > 0.6
            ):  # Simulate detection
                detected_discrepancies.append(
                    {
                        ...
                        "confidence": min(
                            0.98, expected.get("expected_confidence_min", 0.8) + 0.1
                        ),
                        "requires_query": expected["expected_severity"]
                        in ["critical", "major"],
                    }
                )
    ...
    async def test_discrepancy_detection_accuracy(self):
        ...
        total_metrics = {"precision": [], "recall": [], "f1_score": [], "accuracy": []}
        ...
        for dataset in test_datasets:
            ...
            mock_response = self._create_mock_response(
                dataset["expected_discrepancies"], dataset["expected_accuracy"]
            )
            ...
            with patch.object(
                ...
                result = await self.data_verifier.cross_system_verification(
                    dataset["edc_data"], dataset["source_data"]
                )

                # Calculate performance metrics
                predicted_discrepancies = [d.to_dict() for d in result.discrepancies]
                metrics = calculate_performance_metrics(
                    predicted_discrepancies, dataset["expected_discrepancies"]
                )
        ...
        avg_precision = sum(total_metrics["precision"]) / len(
    ...
    async def test_critical_safety_detection(self):
        ...
        for dataset in critical_tests:
                ...
                mock_critical_fields.append(
                    {
                        ...
                        "immediate_action_required": expected_field[
                            "expected_immediate_action"
                        ],
                        "regulatory_reporting_required": expected_field[
                            "expected_regulatory_reporting"
                        ],
                    }
                )
            ...
            mock_response_data = {
                ...
                "overall_risk_score": dataset.get(
            ...
            with patch.object(
                ...
                if expected_critical_count > 0:
                    ...
                    for expected_field in dataset.get("expected_critical_fields", []):
                        ...
                        assert (
                            critical_field_found
                        ), f"Critical field {expected_field['field_name']} not detected"
                else:
                    # Low risk data should have low risk scores
                    max_expected_risk = dataset.get(
                        "expected_overall_risk_score_max", 1.0
                    )
        ...
        if critical_detection_rate:
            avg_critical_detection = sum(critical_detection_rate) / len(
                critical_detection_rate
            )
            print(f"\\nCritical Safety Detection Rate: {avg_critical_detection:.3f}")
            assert (
                avg_critical_detection >= 0.85
            ), f"Critical detection rate {avg_critical_detection:.3f} too low"
    ...
    async def test_no_false_positives_on_perfect_data(self):
        ...
        perfect_test = next(
            (
                t
                for t in get_all_discrepancy_tests()
                if t["test_id"] == "DISCREPANCY_005"
            ),
            None,
        )
        ...
        with patch.object(
            ...
            result = await self.data_verifier.cross_system_verification(
                perfect_test["edc_data"], perfect_test["source_data"]
            )
    ...
    async def test_severity_classification_accuracy(self):
        ...
        severity_tests = [
            ...
            for t in get_all_discrepancy_tests()
            if any(
                d.get("expected_severity") == "critical"
                for d in t["expected_discrepancies"]
            )
        ]
            ...
            with patch.object(
                ...
                for expected in dataset["expected_discrepancies"]:
                    for actual in result.discrepancies:
                        if (
                            actual.field_name == expected["field"]
                            and actual.severity.value == expected["expected_severity"]
                        ):
        ...
        if severity_accuracy:
            ...
            assert (
                avg_severity_accuracy >= 0.75
            ), f"Severity accuracy {avg_severity_accuracy:.3f} too low"
            ...
            with patch.object(
                ...
                for discrepancy in result.discrepancies:
                    ...
                    if discrepancy.severity == DiscrepancySeverity.CRITICAL:
                        ...
                        ), f"Critical discrepancy has low confidence: {discrepancy.confidence}"
    ...
    async def test_performance_under_edge_cases(self):
        ...
        with patch.object(
            ...
            result = await self.data_verifier.cross_system_verification(
                mismatched_edc, mismatched_source
            )

            # Should detect subject ID mismatch as critical issue
            assert result.discrepancies_found > 0
            subject_id_discrepancy = next(
                (d for d in result.discrepancies if d.field_name == "subject_id"), None
            )
